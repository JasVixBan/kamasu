...
__global__ void kamasu_vector_add_tex_knl (struct cublasSscalParams parms);
__global__ void kamasu_vector_add_gld_knl (struct cublasSscalParams parms);
 ...
 */
__host__ void kamasu_vector_scalar_add (int n, float alpha, float *x, int incx)
{
  //    struct cublasContext *ctx = CUBLAS_GET_CTX();
    ...
    */
    ...
    if (useTexture) {
      kamasu_vector_add_tex_knl<<<nbrCtas,threadsPerCta>>>(params);
    } else {
      kamasu_vector_add_gld_knl<<<nbrCtas,threadsPerCta>>>(params);
    }
