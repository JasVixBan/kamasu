    ...
    if (parms.incy == 0) {
        ...
        if (parms.incx == 1) {
            ...
            for (i = ctaStart + tid; i < parms.n; i += totalThreads) {
                sy[i] = fetchy(i) OPERATOR fetchx(i);
            }
        } else {
            /* equal, positive, non-unit increments. */
            for (i = ctaStart + tid; i < parms.n; i += totalThreads) {
                sy[i*parms.incx] = (fetchy(i*parms.incx) OPERATOR fetchx(i*parms.incx));
            }
        }
    } else {
        ...
        for (i = ctaStart + tid; i < parms.n; i += totalThreads) {
            sy[iy+i*parms.incy] = (fetchy(iy+i*parms.incy) OPERATOR fetchx(ix+i*parms.incx));
        }
    }
