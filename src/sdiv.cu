...
__global__ void kamasu_vector_div_tex_knl (struct cublasSscalParams parms);
__global__ void kamasu_vector_div_gld_knl (struct cublasSscalParams parms);
 ...
 */
__host__ void kamasu_vector_scalar_div (int n, float alpha, float *x, int incx)
{
  //    struct cublasContext *ctx = CUBLAS_GET_CTX();
    ...
    */
    ...
    kamasu_vector_splay (n, CUBLAS_SSCAL_THREAD_MIN, CUBLAS_SSCAL_THREAD_MAX,
			 CUBLAS_SSCAL_CTAS_MAX, &nbrCtas, &elemsPerCta, 
			 &threadsPerCta);

    // cudaStat = cudaGetLastError(); /* clear error status */
    if (useTexture) {
      kamasu_vector_div_tex_knl<<<nbrCtas,threadsPerCta>>>(params);
    } else {
      kamasu_vector_div_gld_knl<<<nbrCtas,threadsPerCta>>>(params);
    }
    ...
    if (cudaStat == cudaSuccess) ; // stop warning
