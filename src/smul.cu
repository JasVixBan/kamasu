...
__global__ void kamasu_vector_mul_tex_knl (struct cublasSscalParams parms);
__global__ void kamasu_vector_mul_gld_knl (struct cublasSscalParams parms);
 ...
 */
__host__ void kamasu_vector_scalar_mul (int n, float alpha, float *x, int incx)
{
  //    struct cublasContext *ctx = CUBLAS_GET_CTX();
    ...
    */
    ...
    if (useTexture) {
      kamasu_vector_mul_tex_knl<<<nbrCtas,threadsPerCta>>>(params);
    } else {
      kamasu_vector_mul_gld_knl<<<nbrCtas,threadsPerCta>>>(params);
    }
