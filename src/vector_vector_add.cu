...
__global__ void kamasu_vector_vector_add_tex_knl (struct cublasSaxpyParams parms);
__global__ void kamasu_vector_vector_add_gld_knl (struct cublasSaxpyParams parms);
 ...
 */
__host__ void CUBLASAPI kamasu_vector_vector_add(int n, float* y, int incy, 
				       const float *x, int incx)
{
  //    struct cublasContext *ctx = CUBLAS_GET_CTX();
    ...
    if (useTexture) {
      kamasu_vector_vector_add_tex_knl<<<nbrCtas,threadsPerCta>>>(params);
    } else {
      kamasu_vector_vector_add_gld_knl<<<nbrCtas,threadsPerCta>>>(params);
    }
