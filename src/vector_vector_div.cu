...
__global__ void kamasu_vector_vector_div_tex_knl (struct cublasSaxpyParams parms);
__global__ void kamasu_vector_vector_div_gld_knl (struct cublasSaxpyParams parms);
 ...
 */
__host__ void CUBLASAPI kamasu_vector_vector_div(int n, float* y, int incy, 
				       const float *x, int incx)
{
  //    struct cublasContext *ctx = CUBLAS_GET_CTX();
    ...
    kamasu_vector_splay (n, CUBLAS_SAXPY_THREAD_MIN, CUBLAS_SAXPY_THREAD_MAX,
			 CUBLAS_SAXPY_CTAS_MAX, &nbrCtas, &elemsPerCta, 
			 &threadsPerCta);

    cudaStat = cudaGetLastError(); /* clear error status */
    if (useTexture) {
      kamasu_vector_vector_div_tex_knl<<<nbrCtas,threadsPerCta>>>(params);
    } else {
      kamasu_vector_vector_div_gld_knl<<<nbrCtas,threadsPerCta>>>(params);
    }
