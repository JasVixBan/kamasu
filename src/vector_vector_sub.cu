...
__global__ void kamasu_vector_vector_sub_tex_knl (struct cublasSaxpyParams parms);
__global__ void kamasu_vector_vector_sub_gld_knl (struct cublasSaxpyParams parms);
 ...
 */
__host__ void CUBLASAPI kamasu_vector_vector_sub(int n, float* y, int incy, 
				       const float *x, int incx)
{
  //    struct cublasContext *ctx = CUBLAS_GET_CTX();
    ...
    if (useTexture) {
      kamasu_vector_vector_sub_tex_knl<<<nbrCtas,threadsPerCta>>>(params);
    } else {
      kamasu_vector_vector_sub_gld_knl<<<nbrCtas,threadsPerCta>>>(params);
    }
