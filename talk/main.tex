...
int main (void)
\{
\alert<1>{ \alert<4>{std::}vector<bool> is_prime (100, true);}
\alert<1>{ for (int i = 2; i < 100; i++)}
\alert<2>{    if (is_prime[i])}
\alert<0>{      \{}
\alert<3>{        \alert<4>{std::}cout << i << " ";}
\alert<3>{        for (int j = i; j < 100; i++)}
\alert<3>{             is_prime [j] = false, j+=i);}
\alert<0>{      \}}
\alert<0>{ return 0;}
\alert<0>{\}}
\end{semiverbatim}
