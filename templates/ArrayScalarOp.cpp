...
namespace resophonic {
  namespace kamasu {
			      ...
			      const rk::stream_impl& si)
    {
      ...
      switch (rv.nd) {
	/*'\n'.join(['case %u:  kamasu_elementwise_array_scalar_%d(op_map<%s>::value, rv.data() + rv.offset, rv.linear_size, rv.factors, rv.strides, scalar, si.value); break;' % (this_n, this_n, OP) for this_n in one_to_n])*/
