...
#define ENSURE_NOT_EQUAL(LEFT,RIGHT,...)				\
  I3Test::ensure_not_equal(__FILE__,__LINE__,				\
			   BOOST_PP_STRINGIZE(LEFT),			\
			   BOOST_PP_STRINGIZE(RIGHT),			\
			   LEFT,RIGHT,					\
			   ##__VA_ARGS__);
  ...
  template <typename LeftType, typename RightType>
  inline
  void ensure_not_equal (const string& file, unsigned line, 
			 const string& left_txt, const string& right_txt,
			 const LeftType& left, const RightType& right,
			 const string& msg = "unspecified")
  {
    if(left == right)
      {
	stringstream predstream;
	predstream << "ENSURE_NOT_EQUAL(" << left_txt << ", " << right_txt << "): "
		   << left_txt << " == " << left << ", " 
		   << right_txt << " == " << right;
	throw test_failure(file, line, predstream.str(), msg);
      }
